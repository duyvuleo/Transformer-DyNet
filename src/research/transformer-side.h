...
public:
	...
	dynet::Expression step_forward(dynet::ComputationGraph & cg
		, const dynet::Expression& i_src_rep
		, const dynet::Expression& i_side_rep
		, const WordIdSentence &partial_sent
		, bool log_prob
		, std::vector<dynet::Expression> &aligns
		, float sm_temp=1.f);// forward step to get softmax scores
	...
	dynet::Expression step_forward(dynet::ComputationGraph &cg
		...
		, std::vector<dynet::Expression> &aligns
		, float sm_temp=1.f);
	void sample(dynet::ComputationGraph& cg, const WordIdSentence &source, const WordIdSentence &side, WordIdSentence &target, unsigned length_ratio=2);// random sampling
	void sample(dynet::ComputationGraph& cg, const WordIdSentences &sources, const WordIdSentences &sides, WordIdSentences &targets, unsigned length_ratio=2); // batched version of random sampling
	void sample_sentences(dynet::ComputationGraph& cg
	        , const WordIdSentence& source
		, const WordIdSentence& side
	...
	void sample_sentences(dynet::ComputationGraph& cg
		...
		, const WordIdSentences& sides
  	        , std::vector<WordIdSentence>& samples
		, std::vector<float>& v_probs
		, float softmax_temp=1.f
		, unsigned length_ratio=2);
	void greedy_decode(dynet::ComputationGraph& cg, const WordIdSentence &source, const WordIdSentence &side, WordIdSentence &target, unsigned length_ratio=2);// greedy decoding
	void greedy_decode(dynet::ComputationGraph& cg, const WordIdSentences &sources, const WordIdSentences &sides, WordIdSentences &targets, unsigned length_ratio=2);// batched version of greedy decoding
	void beam_decode(dynet::ComputationGraph& cg, const WordIdSentence &source, const WordIdSentence &side, WordIdSentences &targets, unsigned beam_width, unsigned length_ratio=2, unsigned top_beams=1);// beam search decoding (return top_beams hypos)
	...
	, const WordIdSentences& metas)
{
	...
	// side embeddings	
	std::vector<dynet::Expression> side_embeddings;   
	std::vector<unsigned> words(bsize);		
	for (unsigned l = 0; l < max_len; l++)
	{
		for (unsigned bs = 0; bs < bsize; ++bs)
		{
			if (l < metas[bs].size())
				words[bs] = (unsigned)metas[bs][l];
			else
				words[bs] = (unsigned)pad_ind;
		}
...
void TransformerModel::sample(dynet::ComputationGraph& cg
	, const WordIdSentence &source
	, const WordIdSentence &side
	...
	, unsigned length_ratio)
{
	...
	// get the side info representation
	dynet::Expression i_side_rep = this->encode_meta_info(cg, WordIdSentences(1, side));
	...
	while (target.back() != eos_sym) 
	{
		...
		dynet::Expression ydist = this->step_forward(cg, i_src_rep, i_side_rep, target, false, aligns);
...
void TransformerModel::sample(dynet::ComputationGraph& cg
	...
	, unsigned length_ratio) // batched version of random sampling
{
	_tfc._is_training = false;
	...
	// get the side info representation
	dynet::Expression i_side_rep = this->encode_meta_info(cg, sides);
	...
	while (true) 
	{
		...
		dynet::Expression ydist = this->step_forward(cg, i_src_rep, i_side_rep, targets, false, aligns);// batched
	...
	, unsigned length_ratio)
{
	...
	// get the side info representation
	dynet::Expression i_side_rep = this->encode_meta_info(cg, WordIdSentences(num_samples, side));
	...
	while (true) 
	{
		...
		dynet::Expression i_ydist = this->step_forward(cg, i_src_rep, i_side_rep, samples, false, aligns, softmax_temp);// batched, ((TARGET_VOCAB_SIZE, 1), num_samples)
...
void TransformerModel::greedy_decode(dynet::ComputationGraph& cg
	, const WordIdSentence &source
	, const WordIdSentence &side
	, WordIdSentence &target
	, unsigned length_ratio)
{
	...
	while (target.back() != eos_sym) 
	{
		...
		dynet::Expression i_ydist = this->step_forward(cg, i_src_rep, i_side_rep, target, false, aligns);
...
void TransformerModel::greedy_decode(dynet::ComputationGraph& cg
	...
	, const WordIdSentences &sides
	, WordIdSentences &targets
	, unsigned length_ratio) // batched version of greedy decoding
{
	_tfc._is_training = false;
	...
	while (true) 
	{
		...
		dynet::Expression i_ydist = this->step_forward(cg, i_src_rep, i_side_rep, targets, false, aligns);// batched
...
void TransformerModel::beam_decode(dynet::ComputationGraph& cg
	, const WordIdSentence &source, const WordIdSentence &side
	, WordIdSentences &targets
	, unsigned beam_width, unsigned length_ratio, unsigned top_beams)
{
	...
	for (unsigned steps = 0; completed.size() < beam_width && steps < length_ratio * source.size(); ++steps) {
		...
		for (auto& hprev: chart) {
			...
			dynet::Expression i_ydist = this->step_forward(cg, i_src_rep, i_side_rep, hprev.target, false, aligns);
...
dynet::Dict& TransformerModel::get_side_dict()
{
	return _side_dict;
}
