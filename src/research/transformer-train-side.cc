...
bool load_data(const variables_map& vm
	, WordIdCorpusWithMeta& train_cor, WordIdCorpusWithMeta& devel_cor
...
void run_train(transformer::TransformerModel &tf, 
	const WordIdCorpusWithMeta &train_cor, const WordIdCorpusWithMeta &devel_cor,
	dynet::Trainer*& p_sgd, 
...
// ---
void get_dev_stats(const WordIdCorpusWithMeta &devel_cor
	, const transformer::TransformerConfig& tfc
	, transformer::ModelStats& dstats);
void eval_on_dev(transformer::TransformerModel &tf, 
	const WordIdCorpusWithMeta &devel_cor, 
	transformer::ModelStats& dstats, 
	unsigned dev_eval_mea, unsigned dev_eval_infer_algo);
void eval_on_dev(transformer::TransformerModel &tf, 
	const std::vector<WordIdSentences> &dev_src_minibatch, const std::vector<WordIdSentences> &dev_tgt_minibatch, const std::vector<WordIdSentences> &dev_side_minibatch,  
	transformer::ModelStats& dstats, 
...
int main(int argc, char** argv) {
	...
	opts.add_options()
		...
		("side-train", value<std::string>(), "file containing side/meta information for train")
		("side-devel", value<std::string>(), "file containing side/meta information for devel")
	...
	// print help
	if (vm.count("help") 
		|| !(vm.count("train") && vm.count("devel") && vm.count("side-train") && vm.count("side-devel")))
	{
		cout << opts << "\n";
		return EXIT_FAILURE;
	}
	...
	WordIdCorpusWithMeta train_cor, devel_cor;// integer-converted train and dev data
	...
	if (stat(config_file.c_str(), &sb) == 0 && S_ISREG(sb.st_mode)){// check existence		
		...
		// load data files
		if (!load_data(vm, train_cor, devel_cor, sd, td, side_d, sm))
		...
		if (use_joint_vocab){
			...
			std::string side_vocab_file = model_path + "/" + "side.vocab";
			save_vocab(side_vocab_file, side_d);
		}
		else{
			std::string src_vocab_file = model_path + "/" + "src.vocab";
			std::string tgt_vocab_file = model_path + "/" + "tgt.vocab";
	...
	, SentinelMarkers& sm)
{
	...
	bool use_joint_vocab = vm.count("joint-vocab") | vm.count("shared-embeddings"); 
	if (use_joint_vocab){
		train_cor = read_corpus(train_paths[0], vm["side-train"].as<std::string>(), &sd, &sd, &side_d, true, vm["max-seq-len"].as<unsigned>(), r2l_target & !swap);
		td = sd;
	}
	else
		train_cor = read_corpus(train_paths[0], vm["side-train"].as<std::string>(), &sd, &td, &side_d, true, vm["max-seq-len"].as<unsigned>(), r2l_target & !swap);
	if ("" == vm["src-vocab"].as<std::string>() 
		&& "" == vm["tgt-vocab"].as<std::string>()) // if not using external vocabularies
	{
		sd.freeze(); // no new word types allowed
		td.freeze(); // no new word types allowed
		side_d.freeze(); // no new word types allowed
	}
	if (train_paths.size() == 2)// incremental training
	{
		...
		train_cor = read_corpus(train_paths[1], vm["side-train"].as<std::string>(), &sd, &td, &side_d, true/*for training*/, vm["max-seq-len"].as<unsigned>(), r2l_target & !swap);
	...
	if (vm.count("devel")) {
		...
		devel_cor = read_corpus(vm["devel"].as<std::string>(), vm["side-devel"].as<std::string>(), &sd, &td, &side_d, false/*for development*/, 0, r2l_target & !swap);
	}
	...
	, transformer::ModelStats& dstats)
{
	for (unsigned i = 0; i < devel_cor.size(); ++i) {
		WordIdSentence ssent, tsent, side;
		tie(ssent, tsent, side) = devel_cor[i];  
	...
	unsigned dev_eval_mea, unsigned dev_eval_infer_algo)
{
	if (dev_eval_mea == 0) // perplexity
	{
		...
		for (unsigned i = 0; i < devel_cor.size(); ++i) {
			...
			auto i_xent = tf.build_graph(cg, WordIdSentences(1, ssent), WordIdSentences(1, tsent), WordIdSentences(1, side), nullptr, true);
		...
		for (unsigned i = 0; i < devel_cor.size(); ++i) {
			...
			if (dev_eval_infer_algo == 0)// random sampling
				tf.sample(cg, ssent, side, thyp);// fastest with bad translations
			else if (dev_eval_infer_algo == 1)// greedy decoding
				tf.greedy_decode(cg, ssent, side, thyp);// faster with relatively good translations
			else// beam search decoding
			{
				WordIdSentences thyps;	
				tf.beam_decode(cg, ssent, side, thyps, dev_eval_infer_algo/*N>1: beam decoding with N size of beam*/);// slow with better translations
				thyp = thyps[0];
			}
	...
	unsigned dev_eval_mea, unsigned dev_eval_infer_algo) // batched version of eval_on_dev
{
	if (dev_eval_mea == 0) // perplexity
	{
		...
		for (unsigned i = 0; i < dev_src_minibatch.size(); i++) {		
			...
			const auto& sideb = dev_side_minibatch[i];
		
			dynet::ComputationGraph cg;
			auto i_xent = tf.build_graph(cg, ssentb, tsentb, sideb, nullptr, true);
			losses += as_scalar(cg.forward(i_xent));
		}
		...
		std::vector<MTEval::Sample> v_samples;
		for (unsigned i = 0; i < dev_src_minibatch.size(); ++i) {
			WordIdSentences ssents, tsents, sides;
			ssents = dev_src_minibatch[i];
			tsents = dev_tgt_minibatch[i];
			sides = dev_side_minibatch[i];

			// batched inference/decoding
			dynet::ComputationGraph cg;
			WordIdSentences thyps;// raw translation (w/o scores)
			// FIXME: implement sample and greedy_decode with taking side info into account?
			/*
			if (dev_eval_infer_algo == 0)// random sampling
				tf.sample(cg, ssents, thyps, sides);// fastest with bad translations
			else if (dev_eval_infer_algo == 1)// greedy decoding
				tf.greedy_decode(cg, ssents, thyps, sides);// faster with relatively good translations
			else// beam search decoding
			{
...
{	
	...
	std::vector<std::vector<WordIdSentence> > train_side_minibatch, dev_side_minibatch;
	...
	create_minibatches(train_cor, minibatch_size, train_src_minibatch, train_trg_minibatch, train_side_minibatch);// on train
	cerr << "Creating minibatches for development data (using minibatch_size=" << minibatch_size << ")..." << endl;
	create_minibatches(devel_cor, 1024/*minibatch_size*/, dev_src_minibatch, dev_trg_minibatch, dev_side_minibatch);// on dev
	...
	eval_on_dev(tf, dev_src_minibatch, dev_trg_minibatch, dev_side_minibatch, dstats, dev_eval_mea, dev_eval_infer_algo);// batched version (2-3 times faster)
	...
	while (epoch < max_epochs) {
		...
		for (unsigned iter = 0; iter < dev_every_i_reports;) {
			...
			Expression i_xent = tf.build_graph(cg
					, train_src_minibatch[train_ids_minibatch[id]]
					, train_trg_minibatch[train_ids_minibatch[id]]
					, train_side_minibatch[train_ids_minibatch[id]]
					, &ctstats);
		...
		if (patience > 0 && cpt >= patience)
		{
			if (RESET_IF_STUCK){
				...
				if (USE_SMALLER_MINIBATCH){
					...
					create_minibatches(train_cor, minibatch_size/2, train_src_minibatch, train_trg_minibatch, train_side_minibatch);// on train
					// create a sentence list for this train minibatch
					train_ids_minibatch.resize(train_src_minibatch.size());
					std::iota(train_ids_minibatch.begin(), train_ids_minibatch.end(), 0);
